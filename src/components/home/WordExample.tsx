...
const WordExample = async ({ written, pronounced, meanings, kanji}: WordExampleProps) => {
    ...
    const checkIfFavoritedBefore = async() => {
        ...
        //console.log('is allWordBankWords empty?', allWordBankWords.length)
        //console.log("wordbankwords => ", allWordBankWords)
        // if(allWordBankWords.includes(written)) return true
        // else return false
        if (allWordBankWords.some(obj => obj.written === written)) return true
        else return false

        // allWordBankWords.map( (obj) => {
        //     console.log(obj.written + " == " + written + "?")
        // })
    }
